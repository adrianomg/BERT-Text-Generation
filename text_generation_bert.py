...
def generate_sentence(input_text, num_candidate_words=1):
    """
    ...
    Args:
        ...
        num_candidate_words (int, optional): The number of candidate words to consider. Defaults to 1.
    ...
    top_candidate_words = torch.topk(masked_probs, num_candidate_words, dim=-1).indices.tolist()

    # get random value from 0 to num_candidate_words
    random_index = torch.randint(0, num_candidate_words, (1,)).item()
...
max_candidate_words = 10                # number of generated words each turn to pick from

input_text = "The quick brown fox ran [MASK]." # initial input text

# generate the first word
generated_sentence = generate_sentence(input_text, num_candidate_words=max_candidate_words)
...
for n in range(0, num_words):
    ...
    generated_sentence = generate_sentence(generated_sentence, num_candidate_words=max_candidate_words)
